...
├── darks/     # dark frames (calibrated variants only)
└── bias/      # bias frames (calibrated variants only)
```

Lights-only variants need just `lights/`. If a download opens in the browser
instead of saving, right-click it and choose **"Save link as…"**.
...
| Script | Best for |
| --- | --- |
| **Lights + Bias + Darks + BGE** ⭐ *recommended* | The cleanest starting point for most targets. Full calibration (dark optimization + cosmetic correction), removes the sky gradient from every sub, then runs roundness-filtered registration and stacking. Use when you have matched bias and dark frames. |
| **Lights + Bias + Darks** | Same calibration and stacking, but **skips** background extraction so each frame's gradient stays intact. Choose this if you prefer to remove gradients yourself later. |
...
| Script | Best for |
| --- | --- |
| **Lights + Bias + Darks + BGE** ⭐ *recommended* | The everyday mosaic pipeline. Full calibration + background extraction flattens every tile before stitching so panel borders don't show, then grows the canvas and feathers the seams. Skips overlap normalization, so it's far faster on large panel counts. |
| **Lights + Overlap-Norm + Bias + Darks + BGE** | The highest-quality mosaic, for a final render. Same calibration + BGE **plus overlap normalization** that matches panel levels using only their shared regions for truly seamless joins. This pass dominates runtime — budget well over an hour on 1,000+ frames. |
| **Lights + Bias + Darks** | A calibrated mosaic **without** background extraction to pre-flatten tiles, so seams are more likely. Switch to an overlap-norm variant if panel boundaries show on harder data. |
